...
import { capBoundaryHoles, addPeg, addSocket } from "../../lib/csg";
...
// Single source of truth for which paint group owns a triangle straddling a
// boundary. The old export logic used "any vertex matches" for painted
// groups but "all vertices match" for group 0, so a boundary triangle could
// pass both checks and get written into two different STL files at once
// (overlapping geometry at the seam). Majority vote with a deterministic
// tie-break guarantees exactly one owner per triangle.
function classifyTriangleGroup(g0: number, g1: number, g2: number): number {
  ...
  Object.keys(counts).forEach((key) => {
    ...
    if (count > bestCount || (count === bestCount && groupId < owner)) {
      bestCount = count;
      owner = groupId;
    }
  });
...
export default function Viewer3D() {
  ...
  // Finds where a painted group meets the rest of the model, so a joint can
  // be anchored there automatically. Position = centroid of the group's own
  // boundary vertices (where it touches another group); direction = from
  // that boundary toward the main body's (group 0) centroid, i.e. "into the
  // piece the accessory attaches to". This is a heuristic based on the
  // model's shape, not a user-picked point — for oddly-shaped or multi-armed
  // parts (e.g. a part that touches the main body in two separate places)
  // it collapses to a single averaged point, which may land in a sensible
  // spot or may not. Always eyeball the exported STL before printing.
  const computeGroupBoundaryAnchor = (groupId: number): { position: THREE.Vector3; direction: THREE.Vector3 } | null => {
    if (!modelGeometry) return null;
    const positionAttr = modelGeometry.attributes.position;
    const indexAttr = modelGeometry.index;
    if (!indexAttr) return null; // boundary detection needs triangle adjacency
    ...
    for (let i = 0; i < positionAttr.count; i++) {
      if ((vertexGroups[i] || 0) === 0) {
        mainCentroid.x += positionAttr.getX(i);
        mainCentroid.y += positionAttr.getY(i);
        mainCentroid.z += positionAttr.getZ(i);
        mainCount++;
      }
    }
    if (mainCount === 0) return null;
    mainCentroid.divideScalar(mainCount);

    const boundarySet = new Set<number>();
    for (let i = 0; i < idx.length; i += 3) {
      const a = idx[i], b = idx[i + 1], c = idx[i + 2];
      const ga = vertexGroups[a] || 0, gb = vertexGroups[b] || 0, gc = vertexGroups[c] || 0;
      const touchesGroup = ga === groupId || gb === groupId || gc === groupId;
      const touchesOther = ga !== groupId || gb !== groupId || gc !== groupId;
      if (!touchesGroup || !touchesOther) continue;
      if (ga === groupId) boundarySet.add(a);
      if (gb === groupId) boundarySet.add(b);
      if (gc === groupId) boundarySet.add(c);
    }
    if (boundarySet.size === 0) return null;

    const boundaryCentroid = new THREE.Vector3();
    boundarySet.forEach((vi) => {
      boundaryCentroid.x += positionAttr.getX(vi);
      boundaryCentroid.y += positionAttr.getY(vi);
      boundaryCentroid.z += positionAttr.getZ(vi);
    });
    boundaryCentroid.divideScalar(boundarySet.size);

    const direction = mainCentroid.clone().sub(boundaryCentroid);
    if (direction.lengthSq() < 1e-8) return null; // boundary sits at the main centroid itself — degenerate
    direction.normalize();

    return { position: boundaryCentroid, direction };
  };
      ...
      try {
        ...
        if (indexAttr) {
          ...
          for (let i = 0; i < arr.length; i += 3) {
            ...
            const idx1 = arr[i + 1];
            const idx2 = arr[i + 2];
            const owner = classifyTriangleGroup(vertexGroups[idx0] || 0, vertexGroups[idx1] || 0, vertexGroups[idx2] || 0);
          ...
          const vertexCount = positionAttr.count;
          for (let i = 0; i < vertexCount; i += 3) {
            const owner = classifyTriangleGroup(vertexGroups[i] || 0, vertexGroups[i + 1] || 0, vertexGroups[i + 2] || 0);
            if (owner !== groupId) continue;
            exportPositions.push(
        ...
        if (exportPositions.length === 0) {
          alert("Nenhuma parte do modelo foi pintada com este grupo ainda.");
          setIsExporting(null);
          setIsProcessing(false);
          return;
        }

        // Reconstruct the raw triangle soup, then close the freeform cut
        // boundary so the piece is watertight before any boolean joint op
        // touches it (and before it reaches the slicer either way).
        ...
        // Real joint geometry — the UI's "Tipo de Encaixe" selector used to
        // only change the exported filename. This adds the feature it
        // describes: a snap-fit pin+socket (0.2mm total clearance) or a
        // magnet cavity pair (3.2mm x 1.6mm, for 3x1.5mm neodymium magnets).
        const SNAP_PEG_DIAMETER = 3.0; // mm — adjust to taste per model scale
        const SNAP_PEG_EMBED_DEPTH = 4.0; // mm the pin seats into the mating piece
        const SNAP_FIT_TOLERANCE = 0.1; // mm per side => 0.2mm total clearance
        const MAGNET_DIAMETER = 3.2; // mm, per the UI copy
        const MAGNET_DEPTH = 1.6; // mm, per the UI copy

        if (groupId !== 0) {
          // Accessory piece: gets the male pin (snap-fit) or its own
          // magnet cavity, anchored at its boundary with the main body.
          const anchor = computeGroupBoundaryAnchor(groupId);
          if (anchor) {
            exportGeometry =
              jointType === "magnet"
                ? addSocket(exportGeometry, anchor.position, anchor.direction.clone().negate(), MAGNET_DIAMETER, MAGNET_DEPTH)
                : addPeg(exportGeometry, anchor.position, anchor.direction, SNAP_PEG_DIAMETER, SNAP_PEG_EMBED_DEPTH);
          }
        } else {
          // Main body: carve the matching socket/cavity for every painted
          // accessory that currently exists, not just one.
          groups
            .filter((g) => g.id !== 0 && vertexGroups.some((vg) => vg === g.id))
            .forEach((g) => {
              const anchor = computeGroupBoundaryAnchor(g.id);
              if (!anchor) return;
              exportGeometry =
                jointType === "magnet"
                  ? addSocket(exportGeometry, anchor.position, anchor.direction, MAGNET_DIAMETER, MAGNET_DEPTH)
                  : addSocket(
                      exportGeometry,
                      anchor.position,
                      anchor.direction,
                      SNAP_PEG_DIAMETER + SNAP_FIT_TOLERANCE * 2,
                      SNAP_PEG_EMBED_DEPTH + SNAP_FIT_TOLERANCE
                    );
            });
        }
