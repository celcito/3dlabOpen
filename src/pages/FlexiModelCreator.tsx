...
export default function FlexiModelCreator() {
  ...
  const handleExportSTL = () => {
    try {
      ...
      // Pass 1: build each segment body on its own, no hinge geometry yet.
      const segGeoms: THREE.BufferGeometry[] = [];
      const segFactors: number[] = [];
      const segZ: number[] = [];
      ...
      for (let i = 0; i < segments; i++) {
        ...
        segGeoms.push(segGeom);
        segFactors.push(factor);
        segZ.push(zPos);
      }

      // Pass 2: real peg-and-socket joint between each consecutive pair.
      // The pin is fused (union) into segment i; the matching hole is carved
      // (subtract) out of segment i+1, with hingeGap as the fit tolerance so
      // the two stay mechanically linked but free to rotate after printing.
      for (let i = 0; i < segments - 1; i++) {
        const hSize = (hingeSize / 10) * segFactors[i];
        const hGap = hingeGap / 10;
        const embedDepth = hSize * 1.5; // how far the pin seats inside segment i+1
        const pegLength = segmentGap / 10 + embedDepth;

        const { male, female } = addPegSocketJoint(segGeoms[i], segGeoms[i + 1], {
          position: new THREE.Vector3(0, 0, segZ[i] + currentL / 2),
      ...
      const merged = BufferGeometryUtils.mergeGeometries(segGeoms);
