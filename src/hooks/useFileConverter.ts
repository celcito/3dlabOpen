...
export function useFileConverter() {
  ...
  return { files, outputFormat, isConverting, isLoading: loadingFileId !== null, loadingFileId, fileInputRef, handleFileChange, handleDragOver, handleDrop, removeFile, clearAll, convertBatch, changeOutputFormat, getOutputFilename, pendingCount: files.filter((file) => file.status === "pending" || file.status === "error").length };
}
